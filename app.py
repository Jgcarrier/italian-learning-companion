...
def check_answer(user_answer: str, correct_answer: str, question_type: str = None) -> tuple[bool, str]:
    ...
    - Extra whitespace
    - Sentence translation: More lenient, checks if key words are present
    """
    user_normalized = remove_accents(user_answer.strip().lower())
    correct_normalized = remove_accents(correct_answer.strip().lower())

    # For sentence translation, use more flexible matching
    if question_type == 'sentence_translation':
        # Remove punctuation from both answers
        import string
        user_no_punct = user_normalized.translate(str.maketrans('', '', string.punctuation))
        correct_no_punct = correct_normalized.translate(str.maketrans('', '', string.punctuation))

        # Split into words
        user_words = set(user_no_punct.split())
        correct_words = set(correct_no_punct.split())

        # Remove common words that don't affect meaning
        stopwords = {'the', 'a', 'an', 'is', 'am', 'are', 'was', 'were', 'il', 'lo', 'la', 'i', 'gli', 'le', "l'", 'un', 'una', 'uno'}
        user_words -= stopwords
        correct_words -= stopwords

        # Check if most key words are present (70% threshold)
        if len(correct_words) > 0:
            matches = len(user_words & correct_words)
            similarity = matches / len(correct_words)

            # Accept if 70% of key words match
            if similarity >= 0.7:
                return True, correct_answer

    # Standard exact matching for other types
